 ...
 * provide these to you (alongside source) to pacify any concerns about whether
 * <tt>KDPoint</tt>s have been thoroughly tested.
 * 
...
public class KDPointTests {
	...
	public void testKDPointInt() {
		KDPoint hopefullyOrigin2D = new KDPoint(2), hopefullyOrigin3D = new KDPoint(3);
		...
		for (int i = 0; i < MAX_ITER; i++) {
			...
			try {
				...
				fail("Should've caught a RuntimeException when creating a KDPoint of dimensionality " + currDim
						+ " . Instead we caught a " + t.getClass() + " with message: " + t.getMessage() + ".");
			}
		}
	}
	...
	public void testDistanceKDPoint() {
		...
		for (int i = 0; i < MAX_ITER; i++) {
			...
			try {
				...
				fail("Should've caught a RuntimeException when computing the distance between two KDPoints of dimensionalities + "
						+ currDim + " and " + currDim + 1 + ". Instead, " + "we caught a " + t.getClass()
		...
		assertEquals(Math.sqrt(143.29474049), complexPointOne.distance(complexPointTwo), EPSILON); // Computed
																									// with
																									// Google's
																									// calculator
	...
	public void testDistanceKDPointKDPoint() {
		// Some trivial ones
		assertEquals(0, distance(origin2D, origin2D), EPSILON); // Recall that
																// the static
																// method has
																// been
																// statically
		...
		// And, finally, proper exceptions thrown when comparing objects of
		// different
		// dimensionalities:
		for (int i = 0; i < MAX_ITER; i++) {
			int currDim = r.nextInt(MAX_DIM);
	...
	public void testToString() {
		...
		for (int i = 0; i < MAX_ITER; i++) {
			...
			assertEquals("We failed to generate a proper String-ified representation for " + "the 1D point  #" + i,
					"A KDPoint with coordinates: (" + randNum + ").", p.toString());
			p = new KDPoint(SCALE * randNum);
			assertEquals("We failed to generate a proper String-ified representation for " + "the 1D point  #" + i,
					"A KDPoint with coordinates: (" + SCALE * randNum + ").", p.toString());
		...
		for (int i = 0; i < MAX_ITER; i++) {
			...
			assertEquals("We failed to generate a proper String-ified representation for " + "the 1D point  #" + i,
					"A KDPoint with coordinates: (" + randNums[0] + ", " + randNums[1] + ").", p.toString());
			double[] scaledRandNums = { SCALE * randNums[0], SCALE * randNums[1] };
			p = new KDPoint(scaledRandNums);
			assertEquals("We failed to generate a proper String-ified representation for " + "the 1D point  #" + i,
					"A KDPoint with coordinates: (" + scaledRandNums[0] + ", " + scaledRandNums[1] + ").",
					p.toString());
			double[] minusRandNums = { -randNums[0], -randNums[1] };
			p = new KDPoint(minusRandNums);
			assertEquals("We failed to generate a proper String-ified representation for " + "the 1D point  #" + i,
					"A KDPoint with coordinates: (" + minusRandNums[0] + ", " + minusRandNums[1] + ").", p.toString());
			double[] minusScaledRandNums = { -scaledRandNums[0], -scaledRandNums[1] };
			p = new KDPoint(minusScaledRandNums);
			assertEquals("We failed to generate a proper String-ified representation for " + "the 1D point  #" + i,
					"A KDPoint with coordinates: (" + minusScaledRandNums[0] + ", " + minusScaledRandNums[1] + ").",
					p.toString());
		}

		// Could add tests for more dimensions, but it's not like we will be
		// using toString()
